...
class QAgent:
    ...
    def get_state_index(self, state):
        ...
        '''
        part_x = (self.observation_space_high[0] - self.observation_space_low[0])/self.discrete_sizes[0]
        x_idx = int((state[0] - self.observation_space_low[0])/part_x)
        part_y = (self.observation_space_high[1] - self.observation_space_low[1])/self.discrete_sizes[1]
        y_idx = int((state[1] - self.observation_space_low[1])/part_y)
    ...
    def update(self, state, action, reward, next_state, is_terminal):
        ...
        next_states = self.get_state_index(next_state)
        states = self.get_state_index(state)
        if is_terminal:
            a = np.argmax(self.q_table[next_states[0], next_states[1], :])
            self.q_table[next_states[0], next_states[1], a] = 0
        else:
            action_next = np.argmax(self.q_table[next_states[0], next_states[1], :])
            self.q_table[states[0], states[1], action] += self.alpha*(reward + self.gamma*self.q_table[next_states[0], next_states[1], action_next]- self.q_table[states[0], states[1], action])
    ...
    def get_action(self):    
        ...
        states = self.get_state_index(self.state)
        x = np.random.binomial(1, self.epsilon_decay)
        if x == 0 : 
            return np.argmax(self.q_table[states[0], states[1], :])
        else : 
    ...
    def agent_eval(self):
        ...
        while not done:
            states = self.get_state_index(eval_state)
            action = np.argmax(self.q_table[states[0], states[1], :]) # Take action based on greedy strategy now
